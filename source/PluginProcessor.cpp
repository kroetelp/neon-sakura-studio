...
//==============================================================================
NeonSakuraProcessor::NeonSakuraProcessor()
...
//==============================================================================
juce::AudioProcessorValueTreeState::ParameterLayout NeonSakuraProcessor::createParameterLayout()
{
...
//==============================================================================
void NeonSakuraProcessor::initPresetNames()
{
...
//==============================================================================
void NeonSakuraProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    // Initialize the wavetable engine
...
void NeonSakuraProcessor::releaseResources()
{
    wavetableEngine->releaseResources();
}

bool NeonSakuraProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
...
//==============================================================================
void NeonSakuraProcessor::processBlock(juce::AudioBuffer<float>& buffer,
                                       juce::MidiBuffer& midiMessages)
{
    juce::ScopedNoDenormals noDenormals;

    // Update engine parameters from DAW automation
    updateEngineParameters();

    // Clear buffer
    buffer.clear();

    // Process audio through the wavetable engine
    wavetableEngine->processBlock(buffer, midiMessages);
}

void NeonSakuraProcessor::processBlock(juce::AudioBuffer<double>& buffer,
                                       juce::MidiBuffer& midiMessages)
{
    juce::ScopedNoDenormals noDenormals;
...
//==============================================================================
void NeonSakuraProcessor::updateEngineParameters()
{
    if (!wavetableEngine)
        return;
    ...
    // TODO: LFO parameters will be connected to ModulationMatrix when fully implemented
    // For now, they are exposed for DAW automation but not yet routed internally
}

//==============================================================================
std::shared_ptr<WavetableParams> NeonSakuraProcessor::getWavetableParams()
{
    if (wavetableEngine)
...
//==============================================================================
juce::AudioProcessorEditor* NeonSakuraProcessor::createEditor()
{
    return new NeonSakuraPluginEditor(*this);
}

//==============================================================================
int NeonSakuraProcessor::getNumPrograms()
{
    return programNames.size();
}

int NeonSakuraProcessor::getCurrentProgram()
{
    return currentProgramIndex;
}

void NeonSakuraProcessor::setCurrentProgram(int index)
{
    if (index >= 0 && index < programNames.size())
        currentProgramIndex = index;
}

const juce::String NeonSakuraProcessor::getProgramName(int index)
{
    if (index >= 0 && index < programNames.size())
...
void NeonSakuraProcessor::changeProgramName(int index, const juce::String& newName)
{
    if (index >= 0 && index < programNames.size())
...
//==============================================================================
void NeonSakuraProcessor::getStateInformation(juce::MemoryBlock& destData)
{
    // Store APVTS state using copyState()
...
void NeonSakuraProcessor::setStateInformation(const void* data, int sizeInBytes)
{
    // Restore APVTS state
...
{
    return new NeonSakuraProcessor();
}
